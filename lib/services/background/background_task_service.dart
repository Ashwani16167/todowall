...
Future<void> registerBackgroundTasks() async {
  await Workmanager().registerPeriodicTask(
    'midnight_wallpaper_refresh', 'midnight_wallpaper_refresh',
...
void backgroundTaskDispatcher() {
  Workmanager().executeTask((task, data) async {
    try {
      ...
      if (!Hive.isAdapterRegistered(1)) Hive.registerAdapter(WallpaperSettingsModelAdapter());
      if (!Hive.isBoxOpen(HiveBoxNames.tasks)) await Hive.openBox<TaskModel>(HiveBoxNames.tasks);
      if (!Hive.isBoxOpen(HiveBoxNames.wallpaperSettings)) await Hive.openBox<WallpaperSettingsModel>(HiveBoxNames.wallpaperSettings);
      final tasks = TaskRepositoryImpl(Hive.box<TaskModel>(HiveBoxNames.tasks)).getAllTasks();
      final settings = WallpaperSettingsRepositoryImpl(Hive.box<WallpaperSettingsModel>(HiveBoxNames.wallpaperSettings)).getSettings();
