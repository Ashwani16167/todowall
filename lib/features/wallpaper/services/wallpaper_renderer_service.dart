...
class WallpaperRendererService {
  Future<Uint8List> render(
    ...
    final canvas = ui.Canvas(
        recorder,
        const ui.Rect.fromLTWH(0, 0, WallpaperCanvasUtils.outputWidth,
            WallpaperCanvasUtils.outputHeight));
    canvas.scale(WallpaperCanvasUtils.scale);
    _resolvePainter(settings.template).paint(canvas, tasks, settings);
    final picture = recorder.endRecording();
    final image = await picture.toImage(
        WallpaperCanvasUtils.outputWidth.round(),
        WallpaperCanvasUtils.outputHeight.round());
    try {
